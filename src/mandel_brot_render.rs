    ...
    fn calculate_pixel_cords(&mut self) {
        ...
        for x in 0..self.width {
            for y in 0..self.height {
                ...
                self.pixel_cords.0[index] = cords.x as f32;
                self.pixel_cords.1[index] = cords.y as f32;
            }
        }
    }
        ...
        self.pixel_colors
            ...
            .zip(
                self.pixel_cords
                    .0
                    .array_chunks::<8>()
                    .zip(self.pixel_cords.1.array_chunks::<8>()),
            )
            .par_bridge()
...
fn get_color(iterations: [i32; 8]) -> [Color; 8] {
    let mut colors = [BLACK; 8];
    for i in 0..8 {
...
fn iterat_points(x: [f32; 8], y: [f32; 8]) -> [i32; 8] {
...
impl MultiMandelIterator {
    pub fn new(x: [f32; 8], y: [f32; 8]) -> Self {
        let z_x = f32x8::from_array(x);
        let z_y = f32x8::from_array(y);
        Self {
            z_x,
            z_y,
            z_squared_x: z_x * z_x,
    ...
    fn next_iteration(&mut self) {
        ...
        let new_iterations = abs.lanes_le(f32x8::splat(4.)).to_int();
        self.iteration -= new_iterations; // lane is -1 when abs < 4
    }
    fn get_iterations(&self) -> [i32; 8] {
        self.iteration.to_array()
    }
    fn iterate(&mut self) -> [i32; 8] {
        for _ in 0..128 {
            self.next_iteration();
        }
        self.get_iterations()
